...
- **Not a direct LLM caller.** Baton spawns external runtimes as subprocesses. The only exception is the opt-in escalation advisor (bounded, rate-limited).

### 1.4 Evolution Note

Baton started as a simple task dispatcher ("bridge") but has evolved into a deterministic multi-phase orchestrator (ADR-015). It now includes:
- 16-phase pipeline with complexity-based phase skipping
- Five-role system with tool restriction enforcement (ADR-016)
- Scratchpad, loop detection, and crash recovery (ADR-017)
- Self-annealing feedback loop (ADR-018)

The simple dispatch mode (`baton run`) still works unchanged. Pipeline mode (`baton pipeline run`) adds the phase machine on top.
...
baton CLI (single Go binary)
  |
  +-- Config Loader ---------> agents.yaml (runtimes, phase_machine, role_models, skills, advisor, annealing)
  +-- Spec Validator ---------> .baton/specs/*.yaml
  +-- Prompt Builder ---------> project-brief.md + spec + role + skills -> prompt string
  +-- Task Manager -----------> .baton/tasks/*.yaml (CRUD)
  +-- Lock Registry ----------> .baton/locks.yaml
  +-- Runner -----------------> subprocess lifecycle (spawn, capture, timeout, kill, tool restriction flags)
  ...
  +-- Feedback Miner ---------> event log analysis, pattern detection
  +-- Annealer ---------------> config patch generation from patterns
  +-- TUI Renderer -----------> bubbletea monitor (reads event log via fsnotify)
  |
  +-- spawns subprocesses (one per phase in pipeline mode)
  |
  v
Workers: opencode, claude-code, aider (external binaries)
```
...
### 3.8 Phase Machine (`internal/phase/`)

Deterministic 16-phase pipeline orchestrator. Wraps the existing runner — calls `runner.Run()` once per phase with role-specific prompts.
...
type Pipeline struct {
    ...
    cfg      *config.Config
    spec     *spec.Spec
    // ... store, emitter, manifest, advisor
}
...
Five-role system with boundary verification.
...
Three enforcement levels: prompt injection (all runtimes), runtime flags (supporting runtimes), post-hoc file verification (after completion).

### 3.10 Skill Router (`internal/skill/`)

Loads domain-specific context from `.baton/skills/{domain}/` directories. Reads `.md`, `.txt`, `.yaml`, `.yml` files. Domain resolved from spec field or inferred from context_files extensions.
...
Opt-in LLM consultation when retries exhausted. Falls back to writing context file when disabled or limits exceeded.

```go
type Advisor struct {
    cfg       Config
    runner    Runner       // interface for LLM call
    taskDir   string
    sessionCalls int
...
Response actions: `retry_with_hint`, `skip_phase`, `escalate_to_human`, `modify_constraints`.

### 3.13 Feedback Miner (`internal/feedback/`)

Reads NDJSON event log, computes aggregate metrics, detects actionable patterns.
...
type Analysis struct {
    RuntimePerformance map[string]*RuntimeMetrics
...
### 3.14 Self-Annealer (`internal/annealing/`)

Generates risk-gated config patches from detected patterns.
...
Safety: never auto-patches `phase_machine.enabled` or `escalation_advisor.*`.
