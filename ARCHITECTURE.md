...
- **Three-layer IPC.** Stdout markers for worker→baton (token-efficient), Unix domain sockets for baton↔orchestrator (real-time bidirectional), filesystem for audit trail (crash-safe persistence). See "Communication Architecture" below.
...
- **No frameworks, no heavy dependencies.** Baton is a single Go binary with minimal dependencies (bubbletea for TUI). No LangChain, no CrewAI, no abstractions beyond what the problem requires.
- **macOS + Linux only.** Unix domain sockets are the IPC backbone. Windows is not supported.

## Communication Architecture

Baton uses a three-layer IPC architecture (see [ADR-013](docs/adr/013-unix-socket-ipc.md)):

### Layer 1: Stdout Markers (worker → baton)

Workers print compact text markers to stdout. Designed to be token-efficient since LLM agents pay per character of output:

```
BATON:H:reading codebase              # heartbeat (every 60s)
BATON:P:30:implementing auth          # progress (percent:message)
BATON:S:which schema v1 or v2?        # stuck (blocked, needs help)
BATON:E:build failed                  # error (something broke)
BATON:M:all tests passing             # milestone (subtask done)
```

Baton's runner parses these from the stdout stream and translates them into socket broadcasts + event log entries.

### Layer 2: Unix Domain Socket (baton ↔ orchestrator/CLI)

Per-task socket at `.baton/tasks/{id}/baton.sock`. Minimal JSON-line protocol:
...
Socket lifecycle is tied to `baton run` — created on task start, cleaned up on completion.

### Layer 3: Filesystem Audit Trail

events.ndjson + task.yaml persist all activity for debugging, TUI replay, and crash recovery. The socket is ephemeral; the audit trail is permanent.

### Guidance Delivery to Workers

Workers are unmodified CLI tools that cannot listen on sockets. Guidance flows through a hybrid path:

```
orchestrator → socket → baton run → writes inbox.ndjson → worker reads file
```

The worker's prompt instructs it to check `inbox.ndjson` when stuck. This is the simplest way to deliver data TO an arbitrary CLI worker.

### Why Not gRPC/WebSocket/etc.

| Option | Why rejected |
|--------|--------------|
| gRPC/protobuf | Codegen dependency, overkill for ~5 msg/sec |
| WebSocket | Needs HTTP server, more complex than Unix socket |
| Named pipes | Unidirectional, no multiplexing |
| D-Bus | Linux-only, complex API |
| Pure filesystem | No push, polling latency, no acknowledgment |

Unix domain sockets hit the sweet spot: bidirectional, real-time push, request/reply semantics, Go stdlib only (no external dependencies), per-task isolation.
...
|  - Send guidance via: baton guide <task-id> --msg "..."     |
|  - Monitor via: baton progress <task-id> --watch            |
...
+--------------------+---------------------------------------+
                     | bash commands + Unix socket
+--------------------v---------------------------------------+
|  baton (single Go binary)                                   |
|                                                             |
|  Subcommands:                                               |
|  - run       spawn task + socket server per task            |
|  - status    show all active/completed tasks                |
|  - list      show available runtimes and models             |
|  - result    read the output of a completed task            |
|  - progress  show/stream worker progress markers            |
|  - guide     send guidance to running worker via socket     |
...
|  - Task runner (subprocess + socket server lifecycle)       |
|  - Proto parser (BATON: stdout markers → messages)          |
|  - Socket server (per-task Unix domain socket)              |
...
+--------------------+---------------------------------------+
       ...
       (kimi)     (gemini)   (gpt-4o)
          |          |          |
          | stdout: BATON:P:30:msg
          | stdout: BATON:S:question?
          +----------+----------+
                     v
      .baton/tasks/{id}/baton.sock (real-time)
      .baton/events.ndjson (audit trail)
                     |
                     v
             baton monitor (TUI)
```
...
- **Timeout.** `context.WithTimeout`. Default 60m. On timeout: SIGTERM, wait 5s, SIGKILL.
- **Socket per task.** `baton run` creates `.baton/tasks/{id}/baton.sock`, cleaned up on exit.
...
```
Worker stdout → runner scanner → proto.ParseMarker()
                                      |
                                      ├→ socket.Broadcast() → connected clients
                                      └→ emitter.TaskEvent() → events.ndjson

Socket client (guide cmd) → server.Incoming() → runner.handleIncoming()
                                                        |
                                                        └→ writes inbox.ndjson for worker
```

### Concurrency

- **Event log.** Flock-based serialization for concurrent writes.
- **Task files.** Each task has its own file, no contention. Writes are atomic (temp + rename).
- **Socket server.** One goroutine per client connection. Broadcast protected by RWMutex.

### Output Error Detection

When worker exits 0 but output contains error patterns, baton marks task as failed:
- Scans last 20 lines of output for patterns like "Error:", "panic:", "command not found"
- Catches cases where runtimes (e.g., OpenCode) exit 0 on "Model not found" errors

### Git Integration

```go
// Before spawn: snapshot git state
beforeSnap, _ := gitpkg.TakeSnapshot()
// After completion: detect file changes
...
- Spec validation failure: error listing missing fields.
- Lock conflict: "cannot start task-003: models/session.go locked by task-002."
- Socket creation failure: task still runs, just without real-time socket.
- Worker crash: check clarification patterns first, then mark failed.
- Stale socket files: removed on task completion; stale .sock from crashes cleaned on next run.

### Platform Support

**macOS and Linux only.** Unix domain sockets (`net.Listen("unix", path)`) are the IPC backbone. Windows is explicitly unsupported — no named pipes fallback, no compatibility layer.
...
## Pipeline Architecture (Phase 7-8)

Baton evolved from a simple task dispatcher into a deterministic multi-phase orchestrator. The pipeline wraps the existing runner — `runner.Run()` is called once per phase with a role-specific prompt.

### Phase Machine

The pipeline executes 16 sequential phases with complexity-based skipping:

```
baton pipeline run spec.yaml --complexity MEDIUM
    │
    ├─ For each active phase (skip logic applied):
    │     ├─ Resolve runtime/model for phase's role
    │     ├─ Build phase-specific prompt (base + role + scratchpad + dirty files + skills)
    │     ├─ Call runner.Run() with tool restriction flags
    │     ├─ Parse BATON:C completion promise from output
    │     ├─ On fail → L1 retry (up to max_l1_retries)
    │     ├─ On stuck → loop detection, escalate to L2 or advisor
    │     ├─ On verification fail → L2 loop back to implementation
    │     └─ On done → advance, record dirty files
    │
    └─ Return pipeline result
```

Phases: setup → triage → discovery → skill_discovery → complexity → brainstorming → architecture → implementation → design_verification → domain_compliance → code_quality → test_planning → testing → coverage_verification → test_quality → completion.

TRIVIAL skips to 3 phases (1, 8, 16). SMALL runs 11. MEDIUM/LARGE run all 16.

### Five-Role System

Each phase has a dedicated role with explicit boundaries:

| Role | Phases | Can Do | Cannot Do |
|------|--------|--------|-----------|
| Lead | 1-7, 16 | Read, Grep, Glob, Bash(verify) | Edit, Write, Delete |
| Developer | 8 | Read, Edit, Write, Bash | — |
| Reviewer | 9-11, 15 | Read, Grep, Glob, Bash(verify) | Edit, Write, Delete |
| Test Lead | 12 | Read, Grep, Glob | Edit, Write, Bash |
| Tester | 13-14 | Read, Edit, Write, Bash(test) | Edit production code |

Three enforcement levels:
1. **Prompt injection** — role boundaries in the prompt (works on all runtimes)
2. **Runtime flags** — `--allowedTools` passed to subprocess (runtimes that support it)
3. **Post-hoc verification** — `FilesChanged` checked against role constraints after completion

### Retry and Loop System

```
L1 (intra-phase):  phase fails → retry with scratchpad context → up to max_l1_retries
L2 (inter-phase):  verification fails → loop back to implementation → up to max_l2_cycles
Loop detection:    Jaccard similarity on output tails → if stuck, exhaust L1 early
Advisor:           when everything exhausted → opt-in LLM consultation or context dump
```

**Scratchpad:** Each task gets `.baton/tasks/{id}/scratchpad.md` — accumulates notes across retries via `BATON:N:` markers. Injected into retry prompts. Prevents the Groundhog Day problem.
...
```
.baton/skills/
  backend/conventions.md     → injected when domain=backend or context_files are .go
  frontend/patterns.md       → injected when domain=frontend or context_files are .tsx
```
...
Event log mining → pattern detection → config patch generation:

```
baton feedback        → reads events.ndjson, computes runtime/phase metrics, detects patterns
baton anneal generate → maps patterns to config patches with confidence/risk ratings
```

Detected patterns: runtime_domain_mismatch, retry_budget_insufficient, loop_model_affinity, timeout_mismatch. Safety constraints prevent auto-patching `phase_machine.enabled` or `escalation_advisor`.
